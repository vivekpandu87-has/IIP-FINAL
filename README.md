...
**Board Independence panel** (added after gap-checking against the report's own Section 3.4d spec): the report names board-independence as required dashboard content but never gives a numeric score for it — only the aggregate F.A.M.I.L.Y. total exists per company. Rather than invent a percentage, this panel classifies each of the 8 cases by whether its own case narrative documents a professional/non-family management or board structure, quoting the exact evidence. The one external benchmark added for context:
- Compensation Advisory Partners — *2025 Private Company Board Compensation & Governance Survey* (~50% independent/outside directors is typical for private company boards)

A caveat note was also added next to the Leverage chart: the report's summary language mentions "debt-to-asset ratios," but none of the 4 disclosed companies report a debt-to-asset % in their public figures — no full balance sheets are available to compute one, so the dashboard uses the actual disclosed leverage metrics (net debt/EBITDA, LTV) instead.
