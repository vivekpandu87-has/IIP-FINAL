...
safe("chart-map", () => {
  ...
  function drawMap(metric) {
    ...
    const values = COUNTRY_ROLLUP.map(c => {
      if (!isExposure) return c.avgMaturity;
      // average conflict exposure for the companies in this country
      const inCountry = COMPANIES.filter(comp => c.companies.includes(comp.name));
      const exposures = inCountry.map(comp => Object.values(comp.heat).reduce((s, sev) => s + SEVERITY_WEIGHT[sev], 0));
      return +(exposures.reduce((a, b) => a + b, 0) / exposures.length).toFixed(2);
    });
    Plotly.react(el, [{
      type: "choropleth",
      ...
      hovertemplate: isExposure
        ? "<b>%{text}</b><br>Case(s): %{customdata[0]}<br>Avg. conflict exposure: %{z} / 18<br>GDP (2026): $%{customdata[1]}tn<extra></extra>"
        : "<b>%{text}</b><br>Case(s): %{customdata[0]}<br>Avg. maturity: %{z}/30<br>GDP (2026): $%{customdata[1]}tn<extra></extra>",
      colorscale: isExposure ? [[0, "#EEE9DB"], [0.5, "#C98A2D"], [1, "#7A1F2B"]] : [[0, "#EEE9DB"], [0.5, "#C98A2D"], [1, "#1B2A4A"]],
      zmin: 0, zmax: isExposure ? 18 : 30,
      reversescale: isExposure,
      marker: { line: { color: "#F5F2E9", width: 1.2 } },
      colorbar: { thickness: 12, len: 0.7, tickfont: { family: "IBM Plex Mono", size: 10 }, title: { text: isExposure ? "Exposure" : "Maturity", font: { size: 10 } } },
    }], {
      ...basePlotlyLayout,
...
// ============================================================
// Treemap: revenue (box size) x margin (color) — disclosed cases
// ============================================================
safe("chart-treemap", () => {
  requirePlotly();
  const el = document.getElementById("chart-treemap");
  const merged = REVENUE_DATA.map(r => {
    const m = MARGIN_DATA.find(x => x.name === r.name);
    return { name: r.name, revenue: r.value, margin: m ? m.value : 0, basis: m ? m.basis : "" };
  });
  Plotly.newPlot(el, [{
    type: "treemap",
    labels: merged.map(d => d.name),
    parents: merged.map(() => ""),
    values: merged.map(d => d.revenue),
    text: merged.map(d => `€${d.revenue}bn revenue<br>${d.margin}% margin`),
    textinfo: "label+text",
    textfont: { family: "Inter", size: 13, color: "#fff" },
    marker: {
      colors: merged.map(d => d.margin),
      colorscale: [[0, "#C98A2D"], [1, "#1B2A4A"]],
      cmin: 0, cmax: 45,
      line: { color: "#F5F2E9", width: 2 },
    },
    customdata: merged.map(d => d.basis),
    hovertemplate: "<b>%{label}</b><br>Revenue: €%{value}bn<br>Margin: %{marker.color}%% (%{customdata})<extra></extra>",
  }], { ...basePlotlyLayout, margin: { l: 4, r: 4, t: 4, b: 4 } }, plotlyConfig);
  el.on("plotly_click", data => {
    const c = COMPANIES.find(c => c.name === data.points[0].label);
    if (c) openCompanyModal(c);
  });
});

// ============================================================
// Leverage headroom: actual vs. the company's own stated ceiling
// ============================================================
safe("chart-leverage", () => {
  requirePlotly();
  const el = document.getElementById("chart-leverage");
  Plotly.newPlot(el, [
    { type: "bar", name: "Actual", x: LEVERAGE_DATA.map(d => `${d.company}<br>${d.metric}`), y: LEVERAGE_DATA.map(d => d.actual), marker: { color: "#1B2A4A" }, text: LEVERAGE_DATA.map(d => `${d.actual}${d.unit}`), textposition: "outside", textfont: { family: "IBM Plex Mono", size: 10.5 } },
    { type: "bar", name: "Company's own ceiling", x: LEVERAGE_DATA.map(d => `${d.company}<br>${d.metric}`), y: LEVERAGE_DATA.map(d => d.ceiling), marker: { color: "#C98A2D" }, text: LEVERAGE_DATA.map(d => `${d.ceiling}${d.unit}`), textposition: "outside", textfont: { family: "IBM Plex Mono", size: 10.5 } },
  ], { ...basePlotlyLayout, barmode: "group", showlegend: true, legend: { orientation: "h", y: -0.3, font: { size: 10.5 } }, margin: { l: 34, r: 10, t: 20, b: 70 }, xaxis: { tickfont: { size: 10 } }, yaxis: { gridcolor: "#E7E1D2" } }, plotlyConfig);

  // Add a small note for the net-cash companies, which don't fit a leverage-ratio chart
  const wrap = el.closest(".chart-panel");
  if (wrap && !wrap.querySelector(".net-cash-note")) {
    const note = document.createElement("p");
    note.className = "footnote net-cash-note";
    note.innerHTML = `Hermès and Jerónimo Martins carry no leverage ratio here — both are net-cash positions: ${NET_CASH_COMPANIES.map(n => `<strong>${n.company}</strong> (${n.amount})`).join(", ")}. <em>Note: the report's own summary language mentions "debt-to-asset ratios" as dashboard content, but none of the 4 disclosed companies report a debt-to-asset % — no full balance sheets are available for that calculation. The figures above (net debt/EBITDA, LTV) are the closest real leverage metrics the report actually discloses.</em>`;
...
safe("chart-timeline", () => {
  ...
  const regionColor = { Europe: "#2f5d4f", UAE: "#a9642c" };
  Plotly.newPlot(el, [{
    type: "scatter",
    mode: "markers",
    x: sorted.map(e => e.year),
    y: sorted.map(e => e.company),
    marker: { size: 16, color: sorted.map(e => regionColor[e.region]), line: { color: "#fff", width: 1.5 } },
    text: sorted.map(e => e.event),
    hovertemplate: "<b>%{y}, %{x}</b><br>%{text}<extra></extra>",
  }], {
    ...basePlotlyLayout,
    margin: { l: 130, r: 30, t: 10, b: 40 },
    xaxis: { title: { text: "Year", font: { size: 11 } }, gridcolor: "#E7E1D2", dtick: 5 },
    yaxis: { tickfont: { family: "Inter", size: 12 }, categoryorder: "array", categoryarray: [...new Set(sorted.map(e => e.company))] },
  }, plotlyConfig);
  el.on("plotly_click", data => {
    const e = sorted[data.points[0].pointIndex];
    const c = COMPANIES.find(c => c.name === e.company);
    if (c) openCompanyModal(c);
  });
});

// ============================================================
// UAE vs Europe comparison
// ============================================================
safe("chart-region-maturity", () => {
  requirePlotly();
  const el = document.getElementById("chart-region-maturity");
  Plotly.newPlot(el, [{
    type: "bar",
    x: ["Europe", "UAE"],
    y: [REGION_COMPARISON.maturity.Europe, REGION_COMPARISON.maturity.UAE],
    marker: { color: ["#2f5d4f", "#a9642c"] },
    text: [REGION_COMPARISON.maturity.Europe, REGION_COMPARISON.maturity.UAE].map(v => `${v}/30`),
    textposition: "outside", textfont: { family: "IBM Plex Mono", size: 12 },
  }], { ...basePlotlyLayout, margin: { l: 34, r: 10, t: 20, b: 30 }, xaxis: { tickfont: { size: 13 } }, yaxis: { range: [0, 32], gridcolor: "#E7E1D2" } }, plotlyConfig);
});

safe("chart-region-areas", () => {
  requirePlotly();
  const el = document.getElementById("chart-region-areas");
  const labels = CONFLICT_AREAS.map(a => a.label);
  Plotly.newPlot(el, [
    { type: "bar", name: "Europe", x: labels, y: CONFLICT_AREAS.map(a => REGION_COMPARISON.areaAverages[a.key].Europe), marker: { color: "#2f5d4f" } },
    { type: "bar", name: "UAE", x: labels, y: CONFLICT_AREAS.map(a => REGION_COMPARISON.areaAverages[a.key].UAE), marker: { color: "#a9642c" } },
  ], { ...basePlotlyLayout, barmode: "group", showlegend: true, legend: { orientation: "h", y: -0.35, font: { size: 11 } }, margin: { l: 40, r: 10, t: 20, b: 90 }, xaxis: { tickfont: { size: 10 }, tickangle: -25 }, yaxis: { title: { text: "Avg. severity (0–3)", font: { size: 10.5 } }, gridcolor: "#E7E1D2" } }, plotlyConfig);
});

// ============================================================
// Board Independence & Professionalisation
// ============================================================
safe("independence-benchmark", () => {
  const b = BOARD_INDEPENDENCE_BENCHMARK;
  document.getElementById("independence-benchmark").innerHTML = `
    <div class="context-card" style="border-top-color:#6b3f5e;">
      <div class="context-stat" style="color:#6b3f5e;">${b.stat}</div>
      <div class="context-label">${b.label}</div>
      <div class="context-source"><a href="${b.url}" target="_blank" rel="noopener" style="color:inherit;">${b.source}</a></div>
    </div>
  `;
});

safe("independence-list", () => {
  const documentedCount = BOARD_INDEPENDENCE.filter(b => b.documented).length;
  const rows = BOARD_INDEPENDENCE.map(b => `
    <div class="independence-row ${b.documented ? "documented" : "not-documented"}">
      <div class="independence-name">${b.company}</div>
      <div class="independence-badge">${b.documented ? "Documented" : "Not documented"}</div>
      <div class="independence-evidence">${b.evidence}</div>
    </div>
  `).join("");
  document.getElementById("independence-list").innerHTML = `
    <p class="footnote" style="margin-bottom:14px;">${documentedCount} of 8 case narratives explicitly document a professional or non-family management/board structure.</p>
    ${rows}
  `;
});
...
// ============================================================
// Maturity chart (Plotly) + scale + table
// ============================================================
safe("maturity-chart", () => {
  requirePlotly();
  const el = document.getElementById("maturity-chart");
  const maturitySorted = [...COMPANIES].sort((a, b) => a.maturity.score - b.maturity.score); // ascending, so highest bar ends up on top
  Plotly.newPlot(el, [{
    type: "bar",
    orientation: "h",
    x: maturitySorted.map(c => c.maturity.score),
    y: maturitySorted.map(c => c.name),
    marker: { color: maturitySorted.map(c => SEVERITY_COLOR[c.maturity.level === "Best Practice" ? "Low" : c.maturity.level === "Advanced" ? "Moderate" : c.maturity.level === "Developing" ? "High" : "Critical"]) },
    text: maturitySorted.map(c => `${c.maturity.score}/30`),
    textposition: "outside",
    textfont: { family: "IBM Plex Mono", size: 11 },
    customdata: maturitySorted.map(c => c.maturity.level),
    hovertemplate: "%{y}: %{x}/30 — %{customdata}<extra></extra>",
  }], {
    ...basePlotlyLayout,
    margin: { l: 130, r: 40, t: 10, b: 40 },
    xaxis: { range: [0, 33], gridcolor: "#E7E1D2", title: { text: "score / 30", font: { size: 10.5 } } },
    yaxis: { tickfont: { family: "Inter", size: 12.5 } },
  }, plotlyConfig);
  el.on("plotly_click", data => {
    const c = maturitySorted[data.points[0].pointIndex];
    if (c) openCompanyModal(c);
  });
});
...
window.addEventListener("resize", () => {
  ["chart-map", "chart-region", "chart-maturity-pie", "chart-severity-pie", "chart-revenue", "chart-margin", "chart-ownership", "chart-maturity-bar", "chart-gdp", "chart-benchmark", "chart-correlation", "chart-risk-register", "maturity-chart", "chart-treemap", "chart-leverage", "chart-timeline", "chart-region-maturity", "chart-region-areas"]
    .forEach(id => {
