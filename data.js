...
// --- Governance timeline: real dated events from the case narratives (4.2.1-4.2.8) ---
const TIMELINE_EVENTS = [
  { year: 1930, company: "Al Futtaim", event: "Al Futtaim empire founded", region: "UAE" },
  { year: 1958, company: "Al Naboodah", event: "Founded by brothers Saeed & Mohammed Al Naboodah", region: "UAE" },
  { year: 1960, company: "Al Ghurair", event: "Founded by Saif Ahmed Al Ghurair", region: "UAE" },
  { year: 1982, company: "Al Naboodah", event: "Chairmanship passes to 2nd generation", region: "UAE" },
  { year: 1995, company: "Al Ghurair", event: "Brothers separate the empire into two family holdings", region: "UAE" },
  { year: 2000, company: "Al Futtaim", event: "Formal division between cousins, arbitrated by Sheikh Mohammed bin Rashid", region: "UAE" },
  { year: 2008, company: "Hermès", event: "LVMH begins building a hidden ~23% stake via equity swaps", region: "Europe" },
  { year: 2011, company: "Hermès", event: "Family pools majority block into holding company H51", region: "Europe" },
  { year: 2011, company: "Al Ghurair", event: "Nippon Steel takes 20% stake in Al Ghurair Iron & Steel", region: "UAE" },
  { year: 2013, company: "Hermès", event: "AMF fines LVMH €8m for disclosure breaches", region: "Europe" },
  { year: 2014, company: "Hermès", event: "Settlement returns LVMH's stake to its own shareholders", region: "Europe" },
  { year: 2024, company: "Puig", event: "IPO raises ≈€2.6bn to refinance acquisition debt, preserving family control", region: "Europe" },
];

// --- Leverage headroom: only companies with an explicit ceiling AND an
// explicit actual figure in the report (Table 4.2 + case narratives) ---
// --- Board Independence & Professionalisation ---
// The report names this as required dashboard content (Section 3.4d,
// and the M dimension of F.A.M.I.L.Y.), but the source data never gives
// a per-company numeric board-independence score — only the aggregate
// /30 maturity score exists. Rather than invent a percentage, this
// categorises each company by whether its own case narrative (see
// COMPANIES[].mechanism) documents professional/non-family management
// or board structure. "Not documented" means the evidence isn't stated
// in the case narrative — it does NOT mean the company lacks one.
const BOARD_INDEPENDENCE = [
  { company: "Hermès", documented: false, evidence: "Case narrative describes family control via H51 holding and Émile Hermès SAS; no independent-board detail stated." },
  { company: "Puig", documented: false, evidence: "Case narrative describes post-IPO dual-class family control; no board-composition detail stated." },
  { company: "Haniel", documented: true, evidence: "\"Professional external management\" explicitly documented in the case narrative." },
  { company: "Jerónimo Martins", documented: true, evidence: "\"A listed-company board with family-block control\" explicitly documented." },
  { company: "Al Ghurair", documented: false, evidence: "Case narrative describes the 1990s branch separation; no board-composition detail stated." },
  { company: "Al Futtaim", documented: true, evidence: "\"Professional (largely non-family) management of the retained group\" explicitly documented." },
  { company: "Al Naboodah", documented: false, evidence: "Case narrative describes the two-arm holding structure; no board-composition detail stated." },
  { company: "Al Rostamani", documented: true, evidence: "\"Professional executive structures, portfolio-level oversight\" explicitly documented." },
];

const BOARD_INDEPENDENCE_BENCHMARK = {
  stat: "~50%",
  label: "Independent/outside directors, typical private-company board (median board size: 7)",
  source: "Compensation Advisory Partners, 2025 Private Company Board Compensation & Governance Survey",
  url: "https://www.capartners.com/cap-thinking/private-company-board-compensation-and-governance-survey-2025/",
};


const LEVERAGE_DATA = [
  { company: "Puig", metric: "Net debt / adj. EBITDA", actual: 1.1, ceiling: 2.0, unit: "x", note: "Net debt fell to €1,068m within the IPO listing year" },
  { company: "Haniel", metric: "Loan-to-value (LTV)", actual: 13.5, ceiling: 20, unit: "%", note: "Actual LTV vs. the group's own <20% ceiling" },
];
// Hermès and Jerónimo Martins are net-cash positions (no leverage ratio
// applies) — shown separately in the UI rather than forced onto this chart.
const NET_CASH_COMPANIES = [
  { company: "Hermès", amount: "€11.6bn net cash" },
  { company: "Jerónimo Martins", amount: "€726m net cash (ex-IFRS 16)" },
];

// --- UAE vs Europe regional comparison — computed directly from
// COMPANIES above (maturity averages + per-area severity averages),
// not a separate estimate. ---
const REGION_COMPARISON = (() => {
  const regions = ["Europe", "UAE"];
  const maturity = {};
  const areaAverages = {};
  CONFLICT_AREAS.forEach(a => areaAverages[a.key] = {});
  regions.forEach(region => {
    const inRegion = COMPANIES.filter(c => c.region === region);
    maturity[region] = +(inRegion.reduce((s, c) => s + c.maturity.score, 0) / inRegion.length).toFixed(2);
    CONFLICT_AREAS.forEach(a => {
      const avg = inRegion.reduce((s, c) => s + SEVERITY_WEIGHT[c.heat[a.key]], 0) / inRegion.length;
      areaAverages[a.key][region] = +avg.toFixed(2);
    });
  });
  return { maturity, areaAverages };
})();
